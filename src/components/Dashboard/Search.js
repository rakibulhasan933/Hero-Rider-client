...
import UpdateStatus from './StatusUpdateUser';
	...
	return (
		<>
			<div>
				...
				<div className="flex flex-row my-2">
					<div className="w-full overflow-x-auto">
						<table className="table w-full">
							...
							<tbody>
								{
									result?.map((user) => <UserList key={user._id} setStatusUpdate={setStatusUpdate} user={user} />)
								}
							</tbody>
						</table>
					</div>
				...
				{statusUpdate && <UpdateStatus setStatusUpdate={setStatusUpdate} statusUpdate={statusUpdate} />}
			</div>
		</>
	);
};
